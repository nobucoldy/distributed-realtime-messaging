...
require_once __DIR__ . '/../Applications/Chat/RedisClient.php';

final class QueueTestWebSocketClient
{
    /** @var resource */
    private $socket;
...
function runConsumerOnce(): void
{
    $command = PHP_BINARY . ' ' . escapeshellarg(__DIR__ . '/../Applications/Chat/queue_consumer.php') . ' --once';
    $output = shell_exec($command);
    if ($output === null) {
        throw new RuntimeException('Could not run queue consumer.');
    }
}

$redis = RedisClient::instance();
$redis->del('queue:messages', 'queue:messages:dead');

$roomId = 'room_queue_' . bin2hex(random_bytes(4));
$userOne = 'user_queue_' . bin2hex(random_bytes(4));
$userTwo = 'user_queue_' . bin2hex(random_bytes(4));

$clientOne = new QueueTestWebSocketClient('127.0.0.1', 7272);
$clientTwo = new QueueTestWebSocketClient('127.0.0.1', 7272);

try {
    assertType($clientOne->readJson(), 'connected');
    assertType($clientTwo->readJson(), 'connected');

    $clientOne->sendJson(['type' => 'login', 'uid' => $userOne, 'name' => 'Queue User One']);
    assertType($clientOne->readJson(), 'login_success');

    $clientTwo->sendJson(['type' => 'login', 'uid' => $userTwo, 'name' => 'Queue User Two']);
    ...
    $clientOne->sendJson(['type' => 'group_message', 'room_id' => $roomId, 'content' => 'Hello queue group']);
    assertType($clientOne->readJson(), 'message_queued');
    runConsumerOnce();
    assertType($clientOne->readJson(), 'group_message');
    $groupMessage = $clientTwo->readJson();
    assertType($groupMessage, 'group_message');

    if (($groupMessage['content'] ?? '') !== 'Hello queue group') {
        throw new RuntimeException('Unexpected group message: ' . json_encode($groupMessage));
    }

    $clientOne->sendJson(['type' => 'private_message', 'to_uid' => $userTwo, 'content' => 'Hello queue private']);
    assertType($clientOne->readJson(), 'message_queued');
    runConsumerOnce();
    $privateMessage = $clientTwo->readJson();
    assertType($privateMessage, 'private_message');

    if (($privateMessage['content'] ?? '') !== 'Hello queue private') {
        throw new RuntimeException('Unexpected private message: ' . json_encode($privateMessage));
    }

    $redis->lPush('queue:messages', json_encode([
        'id' => 'bad_job_' . bin2hex(random_bytes(4)),
    ...
    if ($redis->lLen('queue:messages:dead') < 1) {
        throw new RuntimeException('Dead letter queue did not receive failed job.');
    }

    echo "Queue test passed for {$roomId}\n";
} finally {
    $clientOne->close();
    $clientTwo->close();
}
